...
public class OSMLayer implements MapLayer {
    ...
    private AtomicReference<Optional<Future<?>>> lastImageUpdateTask = new AtomicReference<Optional<Future<?>>>(Optional.<Future<?>>absent());
    ...
    @GuardedBy("imageUpdateExecutor thread")
    private LinkedList<Future<?>> loadTasks = new LinkedList<Future<?>>();
    
    private AtomicReference<Optional<BufferedImage>> image = new AtomicReference<Optional<BufferedImage>>(Optional.<BufferedImage>absent());
    ...
    private void updateImage() {
        lastImageUpdateTask.set(Optional.<Future<?>>of(imageUpdateExecutor.submit(new Runnable() {
            @Override
            public void run() {
                
                BufferedImage bimg = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
                
                Optional<Future<?>> lastUpdateTask = lastImageUpdateTask.get();
                if (lastUpdateTask.isPresent()) {
                    lastUpdateTask.get().cancel(true);
                }
                ...
                image.set(Optional.of(bimg));
                
                Optional<MapLayerObserver> observer = mapLayerObserver.get();
                if (observer.isPresent()) {
                    observer.get().notifyImageChanged(OSMLayer.this);
                }
